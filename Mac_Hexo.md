...
1. **使能资源文件夹**
    ...
    在 _config.yml 文件中修改`post_asset_folder`为`true` 。这样做的效果是当使用`hexo new [layout] <title>`来创建文章时，会自动创建一个与文章同名的资源文件夹。所有与文章有关的资源存放在这个关联的资源文件夹中，之后可以通过相对路径来引用它们。

2. **相对路径标签插件**

    利用`asset_img`标签插件，图片会同时出现在文章页和归档页中：
